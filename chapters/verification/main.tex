...
%=======
%Computers have been used for variety of applications in business, science, education, engineering and so on. They help to solve real world problems that would otherwise be slow, impossible or extremely difficult to address without computers and software. However, sometimes they do not behave exactly as we expect them. %\bjcom{skip ``do''}
...
%problems. They can be very hard to detect and correct, %It will become too costly to solve.
%%\bjfix{It will become too costly to solve.}{
...
Some bugs are less serious than others. %In some cases, we can still use software programs with these bugs. 
%For example, bugs appearing in computer games and online news.
%\bjfix{In some cases, we can still use software programs with these bugs. For example, bugs appearing in computer games and online news.}
...
%safety is the most important aspect and we have to ensure that there no bugs.
...
% \bjcom{Break the preceding sentence. You can make several sentences with slightly more content}
%They should allow a large degree of parallelism among accessing threads to minimize serialization bottlenecks, while maintaining the appearance of atomic operations. 
...
Data structures typically use heap-allocated memory to store their data. For example, the concurrent linked queue in java.util.concurrent uses a singly linked list to organize their data. %\bjcom{Fix grammar errors in the preceding sentence}
%\bjcom{Extend the paragraph more by stuff that can complicate data structure implementations, e.g., saying that 
...
%Therefore, we have to find the way to achieve a full coverage of program executions.
%\bjcom{Skip the preceding sentence. You can insert in the beginning of the next: ``Therefore, it is said that.. `` (do not put italics again)} 
