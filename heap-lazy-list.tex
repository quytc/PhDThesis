...
\draw[->] (succ1) -- (cell2);
%\draw[->,dotted,line width=1pt,color=black,out=90,in=120] ($(d1.north)+(0pt,0pt)$) to ($(d2.north west)+(1pt,-1pt)$);

\node[anchor=north,font=\tiny,align=center] at ($(cell2.south)+(0pt,2pt)$) {${\tt p}$};
...
\node[rounded corners,draw = black,name=cell8,minimum width=24pt, minimum height=20pt,anchor=south] at ($(cell7.east)+(164pt,15pt)$){};
...
\node[ellipse callout, fill=yellow!20,anchor=east,inner sep=2pt, 
callout absolute pointer={($(cell8.south west)+(4pt,8pt)$)},draw,,font=\tiny,align=center]  
at ($(cell8.south west)+(-1pt,3pt)$){\tt data \\ \tt value};
%
%
\node[draw = black!50,minimum width=8pt, minimum height=10pt,anchor=north,font=\tiny,inner sep=0pt]at (cell8.north){};
...
%  
\node[draw = black,minimum width=8pt, minimum height=10pt,anchor=south,font=\tiny,inner sep=0pt]at (cell8.south){};
\node[ellipse callout, fill=yellow!20,inner sep=2pt, 
...
\end{tikzpicture}
\caption{A heap configuration of the {\tt Lazy Set} Algorithm where $\tt head$ and $\tt tail$ are global variable, and $\tt p$ and $\tt c$ are two local variables.  The symbols \tick and \cross represent the Boolean values {\tt true} and {\tt false}.}
