...
\begin{tikzpicture}[show background rectangle]
  \begin{scope}[scale=0.5]
    ...
    \node {$\state_i$/$\state_r$} child{edge from parent[draw=none]} child {node {$\state_r$}};
    \node[shift={(3cm,0cm)}] {$\state_i$/$\state_r$} child{node {$\state_r$}} child{edge from parent[draw=none]};
    \node[shift={(5cm,0cm)}] {$\state_t$/$\state_b$} child {edge from parent[draw=none]} child {node {$\state_r$/$\state_t$}};
    \node[shift={(8cm,0cm)}] {$\state_t$/$\state_b$} child {node {$\state_r$/$\state_t$}} child {edge from parent[draw=none]};
    \node[shift={(0cm,-2.2cm)}] {$\state_b$/$\state_t$} child{edge from parent[draw=none]} child {node {$\state_t$/$\state_i$}};
    \node[shift={(3cm,-2.2cm)}] {$\state_b$/$\state_t$} child {node {$\state_t$/$\state_i$}} child {edge from parent[draw=none]};
    \node[shift={(5.25cm,-2.2cm)}] {$\state_i^{leaf}$/$\state_r$};
  \end{scope}
  \begin{scope}[color=white]
    \draw (0cm,-1.5cm) -- +(9cm,0cm);
    \draw (1.5cm,0cm) -- +(0cm,-3cm);
    \draw (4cm,0cm) -- +(0cm,-3cm);
    \draw (6.5cm,0cm) -- +(0cm,-3cm);
  \end{scope}
\end{tikzpicture}
\end{center}
